...
//		loadAdvertiser(custId);
	...
	$('#getBtn').on('click',function(e){
		...
		if(custId==null || custId==undefined || custId==""){
			...
			var source_datas={
				datatype:'json',
				datafields:[{name:'dueAmount'},{name:'paidAmount'}],
				url:'getCustomerOutstanding?custId='+custId,
				async:false
			}
			var dataAdapter_datas = new $.jqx.dataAdapter(source_datas,{autoBind:true,loadComplete:function(data){
				if(data.length>0){
					$('#outstandingAmt').text(data[0].dueAmount);
					$('#paidAmt').text(data[0].paidAmount);
					$('#balanceAmt').text(data[0].balanceAmount);
				}else{
					$('#outstandingAmt').text(0.00);
					$('#paidAmt').text(0.00);

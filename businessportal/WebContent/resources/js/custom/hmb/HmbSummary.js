		...
		rendertoolbar : function(toolbar) {
			...
			viewButton.click(function(event) {
				...
				if (row !== -1) {
					...
					var socity = $('#hmbSummaryGrid').jqxGrid('getcellvalue', row, 'socity');
					var socityName = $('#hmbSummaryGrid').jqxGrid('getcellvalue', row, 'cName');
...
//											+ "<td class='table-header-align-right'>0.00</td>"
					...
					$(".modal-title").text('' + cutOffValue + ' ( ' + dateformat(cutOffFromDate) + ' - ' + dateformat(cutOffTodate) + ' ) Society : '+socity+' - '+socityName);
		...
		columns : [
...

			{
				...
				buttonclick : function(row) {
					...
					$(".modal-title").text('' + cutOffValue + ' ( ' + dateformat(cutOffFromDate) + ' - ' + dateformat(cutOffTodate) + ' ) Society : '+socity+'Society Name : '+socityName);
				}
			},
...
//				aggregates: ["sum"],
	...
	$('#getBtn').on("click", function(e) {
		...
		var source_inventory = {
			datatype : 'json',
			datafields : fields,
			url : 'getHmbSummary?selected_month=' + hmbMonth,
			async : false
		}
